...
class SvValidate {
  ...
  static ValidatorFn<String, String> tryPhone([String error]) {
    return (String value) {
      ...
      if (!RegExp(r'^\d+?$').hasMatch(value) ||
          ...
          //
          (value.startsWith(RegExp("[789]")) && value.length != 10)) {
  ...
  static ValidatorFn<String, String> tryPassword([String error]) {
    return (String value) {
      ...
      if (!value.contains(RegExp(r'^.*(?=.{8,})(?=.*\d)(?=.*[a-zA-Z]).*$'))) {
        return 'Password field requires at least 8 characters with one digit & one alphabet.';
      }
      return null;
    };
  }
